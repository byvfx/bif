...
- **Render-mode picker (Live ↔ Ivar) in breadcrumb row** (2026-05-12). Previously the only way to enter Ivar mode was the one-shot "Ivar Render" button in the render settings panel, and there was no way back to the Vulkan rasterizer until a new stage load. Added a `mode_picker` `QComboBox` next to the AOV picker with entries "Live (Vulkan)" and "Ivar". Selection drives `Renderer::set_render_mode(mode)` which writes `IvarState::mode`; the next frame's render path uses the new mode. The "Ivar Render" trigger remains the way to start a fresh batch — the picker only controls which path is shown.
- **Sky-gradient toggle in breadcrumb row** (2026-05-12). `QCheckBox` "Sky Gradient" controls the Ivar path tracer's blue background (the white→blue `sky_gradient` in `bif_renderer::renderer.rs:423` that fires when escaped rays miss the scene and no HDRI is loaded). Off → escaped rays return the solid `RenderConfig::background` color. Drives `IvarState::use_sky_gradient`, which is read into `RenderConfig` at pass start, so it takes effect on the next bucket dispatched by `trigger_ivar_render`. Two new cxx-qt invokables on `BifShellState` (`sky_gradient_enabled`, `on_set_sky_gradient_enabled`); `Renderer::sky_gradient_enabled()`/`set_sky_gradient_enabled(bool)` delegate. Defaults to ON. WGSL headlight fallback already auto-disables when scene lights exist, so no shader-side toggle was needed.
- **AOV viewer in render view** (2026-05-12). v0.16.5 functional gate. Breadcrumb-row `QComboBox` lists every `AovChannel::all()` entry (Beauty / Alpha / Depth / Normal / Shading Normal / Albedo / Cache Heatmap); selecting a channel sets `IvarState::preview_aov` and the next 16ms tick re-encodes the Ivar overlay via `upload_ivar_pixels`. Pure-Rust `encode_aov_rgba(aov, &IvarState, &ImageBuffer) -> Vec<u8>` extracted out of `upload_ivar_pixels` and unit-tested (7 cases covering pass-through, alpha replicate, depth normalize, normal remap, missing-buffer fallback, heatmap auto-normalize, all-zero heatmap). Cache Heatmap encoding now uses a Turbo colormap polynomial with auto-normalize against the per-frame max sample count (previously fixed-16 black→red→yellow→green ramp). Two small public accessors on `Renderer`: `preview_aov()` getter, `set_preview_aov(aov)` setter. Four new cxx-qt invokables on `BifShellState`: `preview_aov_count`, `preview_aov_name_at`, `active_preview_aov_index`, `on_select_preview_aov`. Session-only state; no QSettings persistence yet.
- **File → Save As wired to a native dialog** (2026-05-12). v0.16.2 close-out punch list. The Save As menu item now opens `QFileDialog::getSaveFileName` pre-filled with the current working-layer identifier (full path + filename), so artists can rename in place or redirect into a different folder without retyping. The chosen path is handed to a new `on_save_as_to_path` invokable that calls `UsdStage::export_layer_as_string` and writes via `std::fs::write`; `.usda` is appended when no extension is supplied. Working-layer identity is unchanged after Save As (export-only) — the binary `.usdc` path is deferred to v0.17 when the `cpp_bridge.rs` split lands. Two regression tests in `crates/bif_core/tests/save_as_roundtrip.rs` lock the export → write → reopen contract.
- **Help → About modal dialog** (2026-05-12). Replaces the status-bar stub with `QMessageBox::about` showing version (`CARGO_PKG_VERSION`), a one-liner project description, and a clickable repo link. Body comes from a new `about_dialog_body` invokable so the version can never drift from `Cargo.toml`. Status bar still gets the short "BIF x.y.z — …" line alongside the dialog.
- **Property Inspector Relationships tab** (2026-05-11). Replaces the Phase C.4 placeholder with a real read-only `QTableView` showing the selected prim's `UsdRelationship`s (name + resolved targets, newline-joined for multi-target rels). Same opinion-color dot delegate as Attributes; per-row tooltip lists the per-layer opinion stack. Adds `UsdBridgeRelationshipData` + `usd_bridge_get_prim_relationships`/`_free` and `usd_bridge_rel_get_opinion_sources` FFI, mirrored as `UsdStage::get_prim_relationships()` and `get_relationship_opinions()` in `bif_core`. Six new `BifShellState` invokables surface the data to Qt. Refreshes on `selected_prim_pathChanged` and `layer_state_revisionChanged`. Authoring (add/remove/retarget) and double-click jump-to-target deferred.
...
- **Edit-target sync failures now surface to the status bar** (2026-05-12). v0.16.2 close-out punch list. Both `on_stage_path_opened` and `set_working_layer` previously called `state.set_edit_target(idx, &stage)` and only `log::warn!` on failure — leaving the user staring at a "Loaded ✓" message while their edits would have silently landed on the wrong layer. The post-load status now appends `⚠ edit target sync failed: …` to the load message, and the layer-stack double-click handler replaces its OK message with `⚠ … — edits may land on the wrong layer`. Stage-mutex poisoning is surfaced the same way. Log paths unchanged; this is purely additional user-visible feedback.
- **Surface MakeVisible ancestor un-hides in the status bar** (2026-05-11). When the user clicks the eye icon to un-hide a prim with invisible ancestors, USD's `MakeVisible()` walks the chain and authors `inherited` on each invisible parent — USD-correct but easily surprising (siblings become visible too). `dispatch_visibility` now returns `(description, unhidden_ancestors)`; the Qt status bar appends `— also un-hiding: /A, /B (+N more)` (truncated at 2; full list to `log::info!`). Ancestor lookup uses composed visibility via existing `get_prim_info_by_path` — over-reports vs `MakeVisible`'s authored-only walk, but accurate enough for UX feedback without an FFI extension.
...
- **Viewport pick → scene browser tree highlight sync** (2026-05-12). Picking a prim in the 3D viewport now scrolls + highlights its row in the scene browser tree. Three bugs ganged up: (1) `find_source_index_for_path` walked the tree via column `ColName` (=1), but `SceneBrowserModel::rowCount` returns 0 for any `parent.column() > 0`, so recursion always saw childless levels — now navigates via column 0 (PathRole is column-agnostic). (2) `selected_prim_pathChanged` connect was guarded by `if (m_state)` even though the panel is always built with a state — guard dropped, signal wires up unconditionally. (3) Initial tree rebuild can race with the scene browser provider returning 0 children for a prim, caching that node as a permanent fake-leaf (`child_count=0, children_populated=true`); added `SceneBrowserModel::refresh_node()` which re-queries state and populates a stuck node when the descent encounters one on the target path. Added a `/BIF/...` synthetic-prefix tolerant `path_matches` helper so picks against loader-synthesized paths still resolve against composed USD-form tree rows.
...
- **Persisted-invisible prims now toggleable on reopen** (2026-05-10). Three bugs ganged up on the visibility round-trip: (1) `usd_bridge_layer_write_visibility` authored `visibility="inherited"` via raw `Set()` — which can't defeat ancestor pruning, so un-hiding a descendant of a hidden parent was a no-op; now uses `UsdGeomImageable::MakeVisible()`/`MakeInvisible()` which walks ancestors and authors `inherited` on each invisible parent. (2) `usd_bridge_load_payloads` skipped its `cache_prim_data` re-run because `prims_cached` was already true from the LoadNone open against an unloaded composition (where `UsdPrimDefaultPredicate` filters out unloaded-payload prims) — the cache stayed empty, scene browser showed 0 roots; fixed by force-resetting the flag and unconditionally re-caching after `stage->Load()`. (3) `loader.rs` skipped invisible meshes at load time so they never became prototypes; toggle-to-visible had no instance to un-hide; now loads all meshes and filters uniformly via `hidden_prim_paths` + `reload_instance_visibility`.
- **Root-layer mute attempts no longer corrupt layer state** (2026-05-10). USD emits a soft `TF_CODING_ERROR` ("Cannot mute cache's root layer") that the Rust side ignored — `layer_state.muted` recorded a phantom mute that replayed on every reload, leaving the scene browser empty until manual unmute. `usd_bridge_stage_mute_layer` now uses `SdfLayer::Find` + handle equality against `GetRootLayer()` to reject the call cleanly with `USD_BRIDGE_ERROR_INVALID_PRIM`.
- **Scene browser deferred-rebuild belt-and-suspenders** (2026-05-10). `SceneBrowserModel` constructor now queues a `QTimer::singleShot(0)` rebuild guarded on `m_root->children.empty()` — covers the case where the model is constructed after the stage-open revision bump has already been emitted (signal queued before connection wired up), without double-resetting when the live signal won the race.

- **Visibility toggle no longer triggers full scene/GPU rebuild** (2026-05-09). Eye icon toggles were calling `reload_working_scene()` which rebuilt all GPU buffers, reloaded textures from disk, and rebuilt the Embree BVH — causing multi-second stalls on texture-heavy scenes. Now dispatches to `reload_instance_visibility()`: filters instance groups and culling via a visibility mask without touching prototype geometry, material tables, textures, or the combined mesh. `EditOperation::Transform` commits skip `reload_working_scene()` entirely (local fast path already writes GPU buffers). `EditOperation::MaterialParamOverride` keeps `reload_working_scene()` for material table rebuild but skips `materials_dirty = true` to avoid texture I/O. Added `full_material_ids`/`full_purposes`/`all_prim_paths` snapshots to `SceneInstances` so filtering is reversible across hide→unhide cycles.

- **Visibility toggle — prim_path, eye icon, tree collapse** (2026-05-08). Three bugs: (1) USD-loaded instances lost `prim_path` during `working_scene` merge — uses `add_instance_with_path` so viewport filtering matches; (2) `CachedSceneGraph` procedural prims shadowed USD mesh prims with stale `is_visible=true` — `CompositeProvider` now merges live visibility from the stage; (3) tree collapsed on visibility click — save/restore expanded paths in `SceneBrowserWidget`, plus recursive `refresh_child_visibility` for non-root eye icon updates.
...
- **Visibility toggle + USDA Apply regression tests** (2026-05-07). Added `visibility_toggle_undo_redo_state` and `usda_apply_visibility_undo_state` roundtrip tests in `edit_op_roundtrip.rs`: both verify live `get_prim_info_by_path().visible` state through apply→undo→redo cycles, complementing the existing layer-text-based tests. Known caveat: geometry-changing USDA Apply (new prims, different meshes) requires a new `extract_scene_from_stage` helper — deferred to v0.17.0.
...
- **C4b-1 Material Sheet tab** (2026-04-27). New Material Sheet tab in the Qt Property Inspector binds editable widgets to the surface shader of the bound material — float/double `QDoubleSpinBox`, int `QSpinBox`, bool `QCheckBox`, color3f/float3 swatch+`QColorDialog` (sRGB→linear at the boundary), token/string `QLineEdit`, all grouped by OpenPBR / UsdPreviewSurface section (Base, Specular, Transmission, Subsurface, Coat, Emission, Geometry, Other). New FFI `usd_bridge_prim_get_bound_material_inputs` enumerates the bound shader's inputs (function-local `thread_local` buffer per the C4a contract). New `dispatch_material_param_override` and `dispatch_material_assign` on `Renderer`, plus `on_set_material_param`/`on_bind_material` qinvokables. Bind action surfaces a path-entry dialog. Round-trip test covers the FFI returning the surface shader path + at least one input.
- **C4b-2 USDA Source dock panel** (2026-04-27). New `View → USDA Source` dock with a monospace `QPlainTextEdit` showing the active edit-target layer's serialized USDA. Apply button validates via `parse_usda` and, on success, dispatches `EditOperation::ReplaceLayerContents` through `Renderer::dispatch_replace_layer_contents` so the swap is one undo step and saves through Ctrl+S. Parse / dispatch errors render in a red status label below the editor. Apply-only validation per the handoff — no real-time keystroke parse. New qinvokables `active_edit_target_layer_text` / `on_apply_usda` route through the C4a save substrate.
- **C4b-3 Shading model dropdown** (2026-04-27). New `QComboBox` in the Material Sheet header surfaces OpenPBR ↔ UsdPreviewSurface. New FFI `usd_bridge_layer_set_shader_id` and `usd_bridge_prim_get_bound_shader_id` author/read the surface shader's `info:id` token. New `EditOperation::SetShaderId` variant + `AttrSlot::ShaderId` slot. `Renderer::dispatch_swap_shading_model` wraps the id swap and best-effort param remap (e.g. `base_color` ↔ `diffuseColor`, `specular_roughness` ↔ `roughness`, `base_metalness` ↔ `metallic`, `specular_ior` ↔ `ior`, `emission_color` ↔ `emissiveColor`) in `EditHistory::begin_group` / `end_group` so a single Ctrl+Z reverts the whole swap. Lossy-param `QMessageBox` warning lists Subsurface/Transmission/Coat as drops when targeting UsdPreviewSurface. Tests: `set_shader_id_roundtrips` and `shading_model_swap_undoes_atomically`.
...
- **C4b code review fixes: USDA Apply rollback + FFI exception hardening** (2026-04-28). `usd_bridge_layer_import_from_string` now snapshots the live layer via `ExportToString` before `TransferContent`, restores from the snapshot if the transfer throws mid-mutation, so a USDA Apply can never leave the active layer in a half-written state. Added defensive `catch (...)` blocks to all new C4a/C4b C-ABI entry points (`_layer_save`, `_layer_export_as_string`, `_layer_import_from_string`, `_parse_usda`, `_layer_get_attr_value`, `_layer_permission_to_edit`, `_layer_set_permission_to_edit`, `_layer_set_shader_input`, `_layer_set_shader_id`, `_prim_get_bound_shader_id`, `_prim_get_bound_material_inputs`) so non-`std::exception` USD throws can never unwind across the FFI boundary. New `import_layer_from_garbage_leaves_layer_intact` regression test locks the rollback contract.
...
- **Viewport LOD toggle — View → Viewport LOD (Ctrl+L)** (2026-04-20). Mirrors the egui-era switch (deleted with `render_ui.rs` in Phase F). Surfaces `Renderer::display_settings.lod_enabled` as a checkable `QAction` + cxx-qt `#[qproperty(bool, lod_enabled)]` on `BifShellState`. New `on_set_lod_enabled(bool)` invokable updates the qprop first (UI responsive even if viewport isn't ready) then mirrors onto `display_settings.lod_enabled` via `with_viewport_mut`; next culling pass honors it (`culling_manager.rs:156-235`). Default-on matches `DisplaySettings::default`. Entry in command palette as "View: Toggle Viewport LOD". Status bar confirms ON / OFF. Disabling is useful when the box-LOD swap is confusing selection debug or geometry inspection.

### Fixed

- **v0.15-qt review blocker (1b) — layer mute wired through the renderer** (2026-04-19). Closes the third review-pass finding that the Layer Stack mute checkbox was UI-only on real stages.
  - **B2 — `set_layer_muted` now drives composition, not just the mirror.** On a real stage (with `current_stage_path`), the invokable mutates `renderer.scene.layer_state.muted` to the new value and re-runs `Renderer::load_usd_scene(&path)`. The loader already snapshots `scene.layer_state.muted` at entry (`scene_loader.rs:1496-1508`) and replays it via `load_usd_with_stage_muted` **before payloads fetch**, so geometry is extracted under the muted composition — no new renderer API needed. Empty-scene case (mute removes the def-providing layer) is handled inside `finalize_usd_scene` (clears viewport, keeps Layer Stack alive for unmute). After reload, refreshes shell mirror from the freshly-composed stage + bumps `layer_state_revision` and `scene_browser_revision` so property inspector / scene tree / color dots re-resolve. Status bar shows `"Layer muted: <identifier>"` or the reload error. Demo-only stacks (no `current_stage_path`) still just update the shell mirror — no regression from Phase C.1 visual testing.
  - **Intentionally does NOT call `reset_scene_state`.** That wipes `scene.layer_state` which would erase the muted snapshot the loader reads on re-entry; `load_usd_scene`/`finalize_usd_scene` already overwrite the USD halves of `SceneManager` and call `reload_working_scene` for the GPU rebuild. Revisit if/when node-graph-through-Qt lands (post-Phase-F restoration) — hybrid setups will want a "reset node caches but preserve layer_state" path.
