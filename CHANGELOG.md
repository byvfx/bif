...
- **v0.16.2 review fixes — relationship leak guard, edit-target sync revert** (2026-05-12). Three follow-ups from the `vfx-code-reviewer` audit of merge `3ef5131`. (1) `usd_bridge_get_prim_relationships` adds a defensive `entries.empty()` early-return before `new UsdBridgeRelationshipData[count]` — symmetric with the existing `rels.empty()` guard, prevents zero-sized-array allocation if either guard is dropped. (2) `SceneBrowserModel::refresh_node` documents the safety contract for callers (begin/endInsertRows on `parent` does not mutate the ancestor's row iteration; the `children.empty()` guard is the load-bearing invariant) and adds a `Q_ASSERT` for debug builds; `TODO(v0.17)` to convert callers to a two-pass pre-populate. (3) `BifShellState::set_working_layer` and the load-stage path now capture the previous `working_layer` index and revert the Qt-side state on `set_edit_target` failure or stage-mutex poisoning — eliminates silent layer-corruption where the UI advertised a layer the C++ stage never adopted. `collect_invisible_ancestors` over-reporting and FFI `TF_VERIFY` thread-id guard remain deferred to v0.17.
