...
| Next | **Phase E.2 moves 2+4 — gated on architecture decision.** `BifShellState` (inside cxx-qt bridge) can't reach `ViewportCallbacks::viewport.renderer.scene` from an invokable — the two Rust singletons live in disjoint scopes. Options: **α** `Arc<Mutex<CommandQueue>>` polled per-frame by ViewportCallbacks (handoff's recommendation, ~50 LOC), **β** raw `*mut ViewportCallbacks` stashed on BifShellState at startup (matches existing `app.rs` pattern, unsafe), **γ** make `Renderer.scene` an `Arc<Mutex<SceneManager>>` and clone into BifShellState (invasive). Move 4 alone can be done cheaply (open a fresh UsdStage from the stored path, read `UsdTimelineData` via existing `UsdStage::get_timeline()` — no shared state). |
