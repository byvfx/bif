...
✅ **Milestones Complete:** 3/7 (43%)  
🎯 **Current State:** RGB triangle rendering with 3D camera  
📦 **Tests Passing:** 26/26  
🚀 **Next Goal:** Interactive camera controls (mouse + keyboard)
...
Make the viewport interactive with mouse orbit and WASD keyboard movement.

**Implementation Plan:**

1. **Mouse Orbit Around Target**
   - Capture `MouseInput` and `CursorMoved` events
   - Convert delta to spherical coordinates
   - Update camera position around fixed target
   - Recalculate view matrix and update uniform buffer

2. **Keyboard Movement**
   - WASD for forward/back/left/right
   - Q/E for up/down (or Space/Shift)
   - Accumulate input state per frame
   - Apply velocity-based movement

3. **Camera Update Method**
   ```rust
   // In bif_math/src/camera.rs
   impl Camera {
...
4. **Event Handling**
   ```rust
   // In bif_viewer/src/main.rs
   WindowEvent::MouseInput { state, button, .. } => {
       // Track mouse button state
   }
   WindowEvent::CursorMoved { position, .. } => {
       // Calculate delta, orbit camera
   }
...
5. **Per-Frame Update**
   - Apply keyboard input to camera position
   - Update uniform buffer with new view-projection
   - Request redraw

**Files to Modify:**
- `crates/bif_viewer/src/main.rs` - Add input event handling
- `crates/bif_math/src/camera.rs` - Add orbit/pan/zoom methods
- `crates/bif_viewport/src/lib.rs` - Expose uniform buffer update

**Estimated Time:** 1-2 hours

**Tests to Add:**
- Camera orbit maintains distance to target
- Keyboard input moves camera correctly
- Clamped pitch prevents gimbal lock

---

### Alternative Milestone 4 Options

**Option 2: Depth Testing + Multiple Objects**
- Add depth buffer to render pipeline
- Render multiple triangles in 3D space
- Test Z-ordering and occlusion
- Time: 1 hour

**Option 3: Mesh Loading (OBJ files)**
