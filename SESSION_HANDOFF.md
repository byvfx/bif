...
- **⚠ TODO — MOVE THE REPO BACK TO `G:`.** The repo **must** live on the server (`G:\__projects\_programming\rust\bif`, SMB `\\freya\D_rename`). It is currently at `C:\__projects\_programming\rust\bif`, a local clone made 2026-08-06 to escape the corruption below — **that was a mistake; the server requirement wasn't known at the time.** Nothing is stranded on `C:`: everything is pushed to GitHub, so moving back is a fresh clone on `G:` + deleting the `C:` copy. **Do this before starting new work.** Steps + mitigations below.
- **Before moving back, mitigate the SMB corruption.** On 2026-08-06 the `G:` checkout corrupted itself three times in one session: `git switch` silently *deleted* two tracked files (`bif_core/src/hdr.rs`, `bif_viewport/src/gizmo.rs`); a `git rebase` reverted five files mid-flight; and `.git/refs/heads/fix/hdri-params` became an undeletable broken ref that made every `git fetch` fatal. Failure mode is **working-tree writes** (checkout/reset/rebase) — reads were fine. Tell-tale: `Test-Path`/`Get-Item` report deleted files as still present (stale metadata) while `git status` correctly shows `D` — **trust `git status`**.
  - Prime suspect is SMB client metadata caching. Try: `Set-SmbClientConfiguration -DirectoryCacheLifetime 0 -FileInfoCacheLifetime 0 -FileNotFoundCacheLifetime 0`
  - Add a Defender exclusion for the repo path — real-time scanning of a network share is a classic cause of file-lock/delete races during checkout.
  - Keep the cargo target dir local (`C:/Users/brandon/.cargo-target`) regardless — SMB is slow for incremental artifacts.
  - After cloning to `G:`, clear the stale CMake caches or `bif_core`'s build script will fail (see the CMakeCache gotcha in CLAUDE.md): `Get-ChildItem $env:CARGO_TARGET_DIR -Recurse -Directory -Filter usd_bridge_build | Remove-Item -Recurse -Force`
  - The old `G:` checkout still has the undeletable broken ref; delete the whole directory server-side rather than trying to repair it.
...
- **Current `C:` clone verified** (so the move back starts from a known-good baseline): full workspace builds (1m23s incl. USD bridge); tests green — bif_core 257, bif_viewport 203, bif_renderer 122, bif_math 76, bif_qt 15; fmt + CI clippy clean.
