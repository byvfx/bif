...
- **Static trace settled half of it without running the app.** Two *separate* bugs, not one:
  1. **Live update never existed in the Qt port.** Spinboxes (`node_graph_widget.cpp:106-114`) had **no `valueChanged` connects** and there was **no bridge fn** for a param-only update. egui emitted `NodeGraphEvent::UpdateHdriParams` (`property_inspector.rs:1818`); the Qt port never got a counterpart. Backend handler (`node_dispatch.rs:118-133`) was correct all along. **FIXED this session.**
  2. **Apply's chain is correct end to end** — panel → bridge → `snarl_load_hdri` → `LoadHdri` → `start_hdri_load` (bakes `to_radians()`+intensity) → `apply_ibl_result` sets GPU params **and** `ivar_state.hdri_rotation/intensity` (`render.rs:373-375`); shaders consume both (`skybox.wgsl:74-75`, `basic.wgsl:368-380`); CPU uses `config.hdri_rotation.unwrap_or(env.rotation())` (`renderer.rs:162-163`). So Apply can only fail via a **silent early return**: `with_viewport_mut(...).unwrap_or(false)` (renderer not ready) or `snarl_load_hdri` → `None` (bad id / wrong type). Neither logged. **Feedback added this session — the fix is also the diagnostic.**
...
- **Next:** (1) **live run** — both env scripts, HDRI from `Z:\_HDRIs\HDRI_Haven\`; scrub rotation (should now update with no Apply), then Apply and read status bar + log (`node_dispatch.rs:109`, `environment_manager.rs:126`, `render.rs:381`). (2) File the intensity/rotation issue once the Apply half is confirmed; PR the branch. (3) Unblock Actions → land #29. (4) Untouched queue: #28 (PointInstancer crash) → #26 (framing) → walk the 5 remaining #22 nodes.
