...
| Current | v0.14.0-dev — Layer-Aware Stage. **Phases A, B, C + D scaffolding + scene-loader wiring + LayerStackPanel complete.** Remaining Phase D: wire panel into Renderer UI layout, opinion inspector extension on `property_inspector.rs`, scene-browser color dots, mute/working-layer dispatch handlers. |
| Commits | `cd87179` bump → `e57b7b8` (last plumbing). (pending) LayerStackPanel. 10+ ahead of origin. |
