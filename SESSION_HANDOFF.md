...
| Next | **Phase E.2 move 2 — gated on architecture decision.** `BifShellState` (inside cxx-qt bridge) can't reach `ViewportCallbacks::viewport.renderer.scene` from an invokable — the two Rust singletons live in disjoint scopes. Options: **α** `Arc<Mutex<CommandQueue>>` polled per-frame by ViewportCallbacks (handoff's recommendation, ~50 LOC), **β** raw `*mut ViewportCallbacks` stashed on BifShellState at startup (matches existing `app.rs` pattern, unsafe), **γ** make `Renderer.scene` an `Arc<Mutex<SceneManager>>` and clone into BifShellState (invasive). Next session locks in α/β/γ, then ships move 2 (load_stage_at_path → SceneManager::load_usd_scene → bump layer_state_revision), then revisits move 4 to use the same shared path (so we stop reopening the stage per click). |
