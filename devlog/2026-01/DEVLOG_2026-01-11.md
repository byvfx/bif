...
Milestone 15: Materials (UsdPreviewSurface + Textures + Disney BSDF)
...
- MaterialUniform struct for GPU (diffuse, metallic, roughness, specular)
- Material bind group (group 1) in render pipeline
- Updated basic.wgsl with PBR shading:
  - Fresnel approximation (Schlick)
  - Metallic/dielectric blend
  - Roughness-based shininess
- Material buffer updates when scene reloads
...
ba84260 docs: update milestones and session handoff for M15 completion
feb3a69 feat: add viewport PBR material support
```
...
| `crates/bif_viewport/src/lib.rs` | MaterialUniform, bind group, PBR pipeline |
| `crates/bif_viewport/src/shaders/basic.wgsl` | PBR shading |
...
- wgpu bind groups: group 0 = camera, group 1 = material
- PBR Fresnel: f0 + (1-f0) * (1-cos)^5

## What Works Now

**Ivar (CPU):**

- Disney BSDF with full PBR properties
- Materials from USD automatically converted

**Viewport (GPU):**

- PBR-inspired shading from material properties
- Fresnel, metallic blend, roughness-based shininess
- UVs available in fragment shader

## Future Work (M16)

- GPU texture upload (texture array or bindless)
- Texture sampling in fragment shader
- Per-instance material IDs
- Normal mapping

## Tests

93+ tests passing (all existing + 5 new Disney BSDF tests + 3 texture tests)

## Wiki Links

- [[shading-materials|Shading and Materials]] — UsdPreviewSurface parsing, Disney BSDF implementation
- [[primvars|Primvars]] — primvar-based per-face material assignment
- [[bif-usd-integration|BIF USD Integration]] — C++ bridge material extraction from USD stage
