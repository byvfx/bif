...
impl UsdaParser {
    ...
    fn parse_def(
            ...
            if let Some(brace_start) = line.find('{') {
                if let Some(brace_end) = line.rfind('}') {
                    ...
                    if let Some((asset_path, target_prim)) = reference_info {
                        return self
                            .parse_inline_reference_content(
                                &path,
                                name,
                                asset_path,
                                target_prim,
                    ...
                    return self
                        .parse_inline_xform_content(&path, name, inline_content, start_line)
        ...
        if let Some((asset_path, target_prim)) = reference_info {
            return self
                .parse_reference_content(&path, name, asset_path, target_prim, start_line)
                .map(|r| Some(UsdPrim::Reference(r)));
        }
        ...
        match prim_type {
            "Xform" => self
                .parse_xform_content(&path, name, start_line)
                .map(|x| Some(UsdPrim::Xform(x))),
            "Mesh" => self
                .parse_mesh_content(&path, name, start_line)
                .map(|m| Some(UsdPrim::Mesh(m))),
            "PointInstancer" => self
                .parse_point_instancer_content(&path, name, start_line)
                .map(|p| Some(UsdPrim::PointInstancer(p))),
            "Scope" => {
                // Scope is like Xform but without transform
                self.parse_xform_content(&path, name, start_line)
                    .map(|x| Some(UsdPrim::Xform(x)))
            }
            _ => {
    ...
    /// e.g., def Xform "Name" (\n    kind = "component"\n)
    fn parse_multiline_metadata_from_def(
        &mut self,
    ...
    fn parse_point_instancer_content(
        &mut self,
        path: &str,
        name: &str,
        start_line: usize,
    ) -> ParseResult<UsdPointInstancer> {
        ...
        loop {
            ...
            // Parse positions
            if (trimmed.contains("point3f[]") || trimmed.contains("float3[]"))
            ...
            // Parse scales
            if (trimmed.contains("float3[]") || trimmed.contains("point3f[]"))
                && trimmed.contains("scales")
            {
    ...
    fn parse_xform_op(&self, line: &str) -> ParseResult<Option<XformOp>> {
        ...
        // xformOp:rotateX = 90
        if line.contains("xformOp:rotateX")
            && line.contains('=')
            && !line.contains("xformOp:rotateXYZ")
        {
